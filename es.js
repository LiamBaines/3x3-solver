...
async function extend() {
  var lineNr = 0;
  var s = fs.createReadStream('corners.csv')
    .pipe(es.split())
    .pipe(es.mapSync(async function (line){
        ...
        console.log('before resume, paused: ' + s.paused)
        s.resume()
        console.log('after resume, paused: ' + s.paused)
    })
...
async function f(state, depth, inputTurn) {
  ...
  reader.resume() // wait until the promise resolves (*)
  //console.log(result); // "done!"
}

async function logSeen(data) {
  let promise = new Promise((resolve, reject) => {
    fs.writeFile('seen.txt', data, function (err) {
        resolve('Appended.');
    })
  })
}
