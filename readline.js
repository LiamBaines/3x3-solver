...
async function processLineByLine() {
  const fileStream = fs.createReadStream('corners.csv');
  ...
  for await (const line of rl) {
    count ++
    seen.add(line.slice(0,8))
    await add(line.slice(0,8), line.slice(9));
  }
  rl.close()
  console.log(`lines iterated: ${count}`)
}
...
async function add(state, depth) {
  return new Promise(resolve => {
    let str = ''
    let L = cube.layout(state, 'abcdefghijkl', '012345')
    const t = ['U', 'u', 'D', 'd', 'L', 'l', 'R', 'r', 'F', 'b']
    t.forEach(element => {
      let toAdd = cube.cornerState(turn(L, element, false).valueOf())
      if (!seen.has(toAdd)) {
        str = str.concat(`\n${toAdd},${parseInt(depth) + 1},`)
      }
    })
    fs.appendFile('corners.csv', str, (err) => {
      if (err) throw err;
    });
    resolve();
  });
