...
async function getStart(m, batchSize) {
  console.log(`getStart running with start point ${m} and batch size ${batchSize}`)
  return new Promise(resolve => {
    fs.readFile('corners.csv', 'utf8', function(err, data){ 
        let arr = data.split(',');
        let returnArray = [];
        for (i = m; i < m + batchSize; i+=2) {
            seen.add(arr[m])
            returnArray.push(arr[m + i], arr[m + i + 1])
        };
        resolve(returnArray); 
    });
  });
...
async function add(solutions) {
  console.log(`adding data of size ${solutions.length}`)
  return new Promise(resolve => {
    fs.appendFile('corners.csv', solutions, function (err) {
      resolve('Appended.');
    });
  });
}
...
async function write(fileName) {
  return new Promise(resolve => {
    fs.writeFile('corners.csv', '', function (err) {
...
async function addMultiple() {
  ...
  let k = 0;
  seen.add('ABCDEFGH')
  while (k != seen.size && k < 8) {
    console.log(`loop started ok with k = ${k}`)
  //for (k = 0; k < n*2; k+=2) {
    //console.log(k)
    //console.log(`k = ${k}`)
    let solutionsToAdd = '';
    ...
    //for (q = 0; q < batchSize; q++) {
      //console.log(`q = ${q}`)
      let start = await getStart(k, batchSize);
      let startPoint = start[0];
      let depth = parseInt(start[1]);
      //console.log(`starting at ${startPoint}, ${depth} for k = ${k} and q = ${q}`)
      let L = cube.layout(startPoint, 'abcdefghijkl', '012345').valueOf()
      //console.log(startPoint, 'abcdefghijkl', '012345')
      for (r = 0; r < 12; r++) {
        let add = cube.cornerState(turn(L, turnsArray[r], false)).valueOf();
        if (!seen.has(add)) {
          solutionsToAdd = solutionsToAdd.concat(`${add},${depth + 1},`);
          //console.log(`${k} ${r}, batchSize: ${batchSize}, solutionsToAdd has length ${solutionsToAdd.length}`)
          seen.add(add);
        };
      }
    }
    //console.log(`After full set of turns, solutionsToAdd has length ${solutionsToAdd.length}`)
    //console.log(solutionsToAdd)
    await add(solutionsToAdd);
    k+=2;
    batchSize+=2;
    //console.log(`batchSize increased to ${batchSize}`)
  }
}
...
function list() {
  ...
  let arr = [];
    while (!finished) {
      arr = cornerSolutions.split(',')
      L = cube.layout(arr[g * 2], 'abcdefghijkl', '012345').valueOf()
      depth = parseInt(arr[g * 2 + 1])
      for (let h = 0; h < 12; h ++) {
        let add = cube.cornerState(turn(L, turnsArray[h], false)).valueOf()
        if (!seen.has(add)) {
          seen.add(add)
          cornerSolutions = cornerSolutions.concat(`${add},${depth + 1},`);
        }
      }
    let progress = seen.size
    console.log(progress)
    if (g == progress || progress > 390625) {
      finished = true;
    } else {
      g++
    }
...
async function log() {
  let val = await getStart(0, 1);
  console.log(val)
  console.log(seen)
}

async function appender() {
  await write('solutions.csv')
  await add('ABCDEFGH,0,');
  await addMultiple(1000);
  console.log(`${seen.size} solutions added.`)
